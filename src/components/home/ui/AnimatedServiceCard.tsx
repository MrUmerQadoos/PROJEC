...
const IconSkeleton = ({ item }: Readonly<{ item: IServiceItem }>) => {
  ...
  useEffect(() => {
    // Looping each sequence individually
    sequence.forEach(([target, keyframes, options]) => {
      // Only spread options if it's an object
      animate(target, keyframes, {
        ...(typeof options === 'object' ? options : {}), // Safely spreading options if it's an object
        repeat: Infinity, // Repeating each sequence infinitely
